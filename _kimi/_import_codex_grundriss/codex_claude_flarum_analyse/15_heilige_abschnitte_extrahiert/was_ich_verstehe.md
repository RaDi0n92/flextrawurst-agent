...
Ich habe `was_ich_verstehe.md` als Aggregation des Abschnittstyps `Was ich verstehe` gelesen. Das heißt: Die Datei liest nicht Flarum direkt, sondern sammelt ausgeschnittene Pflichtabschnitte aus vielen Analyse-Dateien.
...
Ich verstehe `Was ich verstehe` hier als Navigator. Die Datei sammelt Verstehenssätze, nicht bestätigte Wahrheiten.
...
Ich verstehe aus `was_ich_verstehe.md` nicht, ob ein einzelner Eintrag im Ursprung stark, schwach, ironisch, warnend oder nur formal war.

Die Aggregation kann Kontextnähe anzeigen, aber keine Kontextbedeutung entscheiden.

## Was mich interessiert

Mich interessiert bei `Was ich verstehe`, welche Muster über viele Dateien hinweg auftauchen. Wiederholt sich eine echte Sorge, eine Baufrage, eine Sperre, ein Missverständnis?

Aber jedes interessante Muster muss zurück in die Ursprungsdateien springen, sonst wird aus Navigation wieder Nebel.

## Was zusammenhängt und wie

`was_ich_verstehe.md` hängt mit allen Ursprungsdateien zusammen, aus denen seine Einträge extrahiert wurden. Es hängt außerdem mit `MANIFEST.md` und `INDEX.md` zusammen, weil diese beiden erklären müssen, dass `15` kein Quellenordner ist.
...
Konzeptionell steht hier eine Suchtechnik: gleiche Abschnittsfrage über viele Dateien legen. `Was ich verstehe` ist also ein Filter, keine Stimme.
...
Mich beschäftigt bei `Was ich verstehe` genau Daniels Kritik: Die Form darf das Denken nicht fressen.
...
Tiefer betrachtet zeigt `Was ich verstehe`, warum die heiligen Abschnitte zweischneidig sind. Sie erzwingen Reflexion, aber sie können auch Reflexion simulieren.
...
`was_ich_verstehe.md` existiert, weil Daniel einzelne benannte Extrahiererdateien wollte. Das ist sinnvoll, wenn man quer über die Analyse nach `Was ich verstehe` suchen will.

Die Datei existiert nicht, damit dieser Abschnittstyp als neues Analysekapitel ohne Ursprung gelesen wird.

## Was ich beim Bauen brauche

Beim Bauen brauche ich für `Was ich verstehe` eine harte UI-Regel: Jeder Eintrag zeigt zuerst den Quellenpfad und einen Button `Quelle öffnen`.
...
**Vision-Schicht:** `Was ich verstehe` ist ein Navigatorregal. Es zeigt ausgeschnittene Abschnitte aus vielen Dateien nebeneinander, aber die Bedeutung bleibt in der Ursprungsdatei.

**Code-Skizze:**
```ts
interface HolySectionEntry {
  section: 'Was ich verstehe';
  sectionKey: 'was_ich_verstehe';
  sourcePath: string;
  sourceTitle: string;
...
Merken will ich mir für `Was ich verstehe`: Aggregation ist Suchhilfe, nicht Erinnerung.

Wenn ein Satz hier wichtig wirkt, ist der nächste richtige Schritt nicht Deutung, sondern Quelle öffnen.

## Dokumente gehören zusammen

`was_ich_verstehe.md` gehört zu seinem Abschnittstyp, zum Manifest, zum Index und zu allen Ursprungsdateien.
...
Die Resonanz von `Was ich verstehe` ist Suchresonanz. Viele gleichartige Abschnitte nebeneinander können Muster erzeugen.
...
`Was ich verstehe` darf diese Schichten nicht überspringen.
...
Vergessen werden soll bei `Was ich verstehe` die Idee, dass Aggregation automatisch Tiefe erzeugt.
...
Bis dahin bleibt `was_ich_verstehe.md` eine manuell markierte Suchdatei mit Quellenpflicht.
