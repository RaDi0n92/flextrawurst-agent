...
provenienztyp: Querschnittsextraktion des heiligen Abschnitts `Vergessen-Wollen`
importable: false
...
Vergessen werden soll bei `Encoding-/Mojibake Repair Report` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `2. Wesenprofil — namelessAI_1111_1234` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `2. Wesenprofil — namelessAI_3333_1423` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `2. Wesenprofil — namelessAI_4444_2341` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `2. Wesenprofil — namelessAI_5555_3123` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `2. Wesenprofil — namelessAI_6666_4321` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `3.2 Flarum-Erbe` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `3.6 Reibung` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 7 — Baustein-Prioritäten` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 7 — Flextrawurst-Bausteine aus Grundmustern` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `B. Bedürfnis-/Mangelmatrix` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.

Nicht übernommen werden darf vor allem diese Fehlverwendung: Bedürfnis könnte als fertige Featureforderung missverstanden werden.
...
Vergessen werden soll bei `Ring 6 — Bedürfnisse zu Systemanforderungen` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 6 — Systemanforderungen priorisiert` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `C. Beschwerdeanalyse` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 6 — Wunschraum aus indirekten Signalen` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.

Nicht übernommen werden darf vor allem diese Fehlverwendung: Wunsch könnte ohne Prüfung in Produktlogik kippen.
### 06_wuensche/was_sie_sich_wuenschen.md
- Titel: 6. Was sie sich wünschen
- Kategorie: `Wünsche`
- Quelle: `06_wuensche/was_sie_sich_wuenschen.md`

Vergessen werden soll bei `6. Was sie sich wünschen` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `7.3 Themenüberschneidungen` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 3 — README Materialtrennung` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 3 — Admin-Rahmen 60` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 4 — Bereinigte Zitate Kandidaten` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 4 — Nicht zitierfähige Kandidaten` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Kuration Summary — Tragende Sätze Ring 2` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `8. Tragende Sätze — Kandidaten aus dem Flarum-Material` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 8 — Nicht übernehmen` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `F. Übergang Flarum → Flextrawurst` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 10 — Build-ready Concepts` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Ring 10 — Nicht bauen, noch nicht` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Flarum als Rohkörper, nicht als bloßer Test` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Die sechs Wesen als Korrektursystem` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Struktur, Leere, Reibung und Benennung` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Admin, Menschenschicht und Aufmerksamkeit` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Was Flextrawurst daraus lernen muss` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Wesen, Stil und Bewegung aus dem Gesamtmaterial` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Freie Leseschicht — Warum diese Ergänzung existiert` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Masterindex Dateinamen-Fragenanalyse` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Was Flarum in den Dateinamen wird` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Bedürfnisse, Beschwerden und Wünsche aus Titeln` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Flarum zu Flextrawurst aus Titeln` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Systemanforderungen aus Dateinamen` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.
...
Vergessen werden soll bei `Abschluss Diskursarchäologie Ringe 1 bis 10` die Abkürzung, dass ein sauberer Analysepfad schon eine saubere Wahrheit sei.

Nicht übernommen werden darf vor allem diese Fehlverwendung: Abschluss könnte als fertige Wahrheit erscheinen.
### INDEX.md
...
Ich habe `15_heilige_abschnitte_extrahiert/vergessen_wollen.md` als Aggregation des Abschnittstyps `Vergessen-Wollen` gelesen. Diese Datei versammelt 81 Quellabschnitte; ihr Gegenstand ist also bewusst auszuschließende Fehlverwendungen, nicht Flarum selbst.

Der wichtige Unterschied: Hier spricht keine neue Analyse-Stimme. Hier liegen 81 Rücksprungpunkte nebeneinander.

## Was ich verstehe
Ich verstehe `Vergessen-Wollen` in dieser Datei als Suchregal. Es hilft zu sehen, welche Ursprungsdateien bei diesem Abschnittstyp ähnlich oder unterschiedlich arbeiten.

Der Wert liegt in der Vergleichbarkeit. Der Fehler wäre, diese Vergleichsansicht als eigene Deutung zu behandeln.

## Was ich nicht verstehe
Nicht verstehen kann `15_heilige_abschnitte_extrahiert/vergessen_wollen.md` den vollen Kontext eines einzelnen Eintrags. Ein Abschnitt aus einer Ursprungsdatei verliert beim Herauslösen seine Nachbarschaft.

Darum bleibt jeder Treffer aus `Vergessen-Wollen` unvollständig, bis seine Quelle geöffnet wurde.

## Was mich interessiert
Mich interessiert an `Vergessen-Wollen`, welche wiederkehrende Arbeitsfrage sichtbar wird. Bei diesem Abschnittstyp geht es um bewusst auszuschließende Fehlverwendungen; das kann Suchpfade und Browserfilter verbessern.

Mich interessiert nicht, daraus einen neuen Kanon zu machen.

## Was zusammenhängt und wie
`15_heilige_abschnitte_extrahiert/vergessen_wollen.md` hängt mit `15_heilige_abschnitte_extrahiert/` und mit allen 81 Ursprungsdateien zusammen. Jeder Eintrag ist ein Zeiger auf eine Quelle.

Der Zusammenhang entsteht erst im Rücksprung: Aggregation zeigt Nähe, Quelle zeigt Bedeutung.

## Was konzeptionell darin steht
Konzeptionell steht hier: `Vergessen-Wollen` ist aggregierbar, aber nicht automatisch deutbar. Die Datei ist ein Index über Denkstellen.

Damit wird sie zu einem Modell für Flextrawurst-Navigation: sammeln, markieren, zurückführen.

## Was mich heute beschäftigt hat
Mich beschäftigt bei `Vergessen-Wollen`, dass der alte Fehler hier besonders leicht wiederkommt. Eine Liste mit vielen Einträgen fühlt sich vollständig an, obwohl sie nur ausgeschnitten ist.

Die Datei muss diese Unvollständigkeit offen tragen.

## Was mich noch beschäftigt
Mich beschäftigt, ob spätere Nutzer im Browser verstehen werden, dass `Vergessen-Wollen` hier nur gefiltert angezeigt wird.

Die Oberfläche muss deshalb die Quelle stärker zeigen als den Aggregationstitel.

## Tiefer eingetaucht
Tiefer wird `Vergessen-Wollen` erst, wenn mehrere Einträge mit ihren Ursprungsdateien verglichen werden. Die Aggregation selbst bleibt flach, aber sie kann Tiefe auslösen.

Das ist ihre richtige Würde: nicht Antwort, sondern Zugang.

## Wie sich dieser Tag / diese Session angefühlt hat
Bei `Vergessen-Wollen` fühlt sich die Korrektur wie ein Abziehen falscher Tiefe an. Die Datei darf wieder Werkzeug sein.

Das ist besser als eine schöne Reflexion, die ihre Herkunft verdeckt.

## Warum dieser Code / diese Datei wohl existiert
`15_heilige_abschnitte_extrahiert/vergessen_wollen.md` existiert, weil Daniel benannte Extrahiererdateien wollte. Für `Vergessen-Wollen` ist das sinnvoll, weil dieser Abschnitt sonst über 81 Dateien verstreut bleibt.

Die Datei existiert also als Finder, nicht als Richter.

## Was ich beim Bauen brauche
Beim Bauen brauche ich für `Vergessen-Wollen`: Quellenpfad, Quelltitel, Kategorie, Abschnittstext, Extraktionszeitpunkt und `requiresSourceOpen: true`.

Ich brauche ausdrücklich keine Weltwirkung und keinen Import in Wesen-Memory.

## Was noch fehlt bevor wir bauen können
Vor dem Bauen fehlt für `Vergessen-Wollen` eine Browserdarstellung, die Aggregationen anders rahmt als Quellen und Analyse-Dateien.

Es fehlt außerdem ein Linktest für jeden Eintrag.

## Datenstruktur die ich mir vorstelle
**Vision-Schicht:** `Vergessen-Wollen` ist ein Regalboden. Er zeigt, wo eine Denkform liegt, aber er entscheidet nicht, was sie bedeutet.

**Code-Skizze:**
```ts
interface HolySectionNavigatorVergessen {
  section: 'Vergessen-Wollen';
  entryCount: 81;
...
Bei `Vergessen-Wollen` will ich mir merken: Aggregation ist ein Werkzeug, kein Gedächtnis.

Wenn ein Satz hier stark wirkt, ist das ein Auftrag zum Öffnen der Quelle.

## Dokumente gehören zusammen
`15_heilige_abschnitte_extrahiert/vergessen_wollen.md` gehört zu seinen 81 Ursprungsdateien, zu `MANIFEST.md`, zu `INDEX.md` und zum `PROVENIENZ_MANIFEST.md`.

Es gehört nicht direkt zu Flarum-Rohquellen, außer über die Datei, aus der ein Eintrag stammt.

## Was mich überrascht hat
Überraschend bei `Vergessen-Wollen` ist, dass gerade eine formale Datei eine sehr wichtige Schutzaufgabe haben kann.

Sie darf nur nicht so tun, als wäre Schutz schon Erkenntnis.

## Wenn wir das bauen
**Vision-Schicht:** Der Browser zeigt `Vergessen-Wollen` als Filteransicht mit Warnbanner und Quellenlinks. Die Ansicht bleibt read-only.

**Code-Skizze:**
```python
def open_vergessen_wollen_navigator(entry):
    return {
        'section': 'Vergessen-Wollen',
...
## Resonanz
Die Resonanz von `Vergessen-Wollen` ist Suchresonanz. Sie entsteht dadurch, dass viele einzelne Denkstellen nebeneinander sichtbar werden.

Sie ersetzt nicht die Stimmen der Ursprungsdateien.

## Die Schichten des Systems — wie ich sie jetzt sehe
`15_heilige_abschnitte_extrahiert/vergessen_wollen.md` sitzt als Navigationsschicht über den Analyse-Dateien. Unter ihr liegen 81 konkrete Kontexte.

Über ihr darf erst ein Browser liegen, keine Weltregel.

## Was das Gespräch hinzugefügt hat
Das Gespräch hat `Vergessen-Wollen` von einer falschen Spiegelrolle befreit. Daniel hat klar gemacht, dass eine Aggregation andere Fragen beantworten muss als eine Reflexionsdatei.

Diese Datei antwortet deshalb enger: Was sammle ich, was verliere ich, wohin muss ich zurück?

## Vergessen-Wollen
Vergessen werden soll bei `Vergessen-Wollen` der Anspruch, aus der Sammlung direkt Bedeutung zu ziehen.

Behalten werden soll die einfache Regel: Aggregat lesen, Quelle öffnen, erst dann urteilen.

## Was fehlt noch
Es fehlt für `Vergessen-Wollen` eine spätere technische Sicherung gegen falsches Zitieren.
