#!/usr/bin/env python3
"""
Extrahiert heilige Abschnitte aus Codex-Dateien.
Jeder Abschnitt landet in seiner eigenen Datei unter _codex/resonanz/<abschnitt>.md
...
def schreibe_in_dimension(dateiname: str, heading: str, inhalt: str, quelle: str, datum: str):
    ...
    if not pfad.exists():
        ...
        pfad.write_text(
            f"# {titel}\n\n"
            f"Wächst automatisch. Jeder Eintrag kommt aus einer Codex-Datei.\n\n",
            encoding="utf-8",
        )
    else:
...
def main():
    ...
    rel = str(filepath).replace("/root/werkraum/_codex/", "")
