...
provenienztyp: Querschnittsextraktion des heiligen Abschnitts `Was ich beim Bauen brauche`
importable: false
...
Ich habe `was_ich_beim_bauen_brauche.md` als Aggregation des Abschnittstyps `Was ich beim Bauen brauche` gelesen. Das heißt: Die Datei liest nicht Flarum direkt, sondern sammelt ausgeschnittene Pflichtabschnitte aus vielen Analyse-Dateien.
...
Ich verstehe `Was ich beim Bauen brauche` hier als Navigator. Die Datei sammelt Bauhinweise, aber keine Bauaufträge.
...
Ich verstehe aus `was_ich_beim_bauen_brauche.md` nicht, ob ein einzelner Eintrag im Ursprung stark, schwach, ironisch, warnend oder nur formal war.

Die Aggregation kann Kontextnähe anzeigen, aber keine Kontextbedeutung entscheiden.

## Was mich interessiert

Mich interessiert bei `Was ich beim Bauen brauche`, welche Muster über viele Dateien hinweg auftauchen. Wiederholt sich eine echte Sorge, eine Baufrage, eine Sperre, ein Missverständnis?

Aber jedes interessante Muster muss zurück in die Ursprungsdateien springen, sonst wird aus Navigation wieder Nebel.

## Was zusammenhängt und wie

`was_ich_beim_bauen_brauche.md` hängt mit allen Ursprungsdateien zusammen, aus denen seine Einträge extrahiert wurden. Es hängt außerdem mit `MANIFEST.md` und `INDEX.md` zusammen, weil diese beiden erklären müssen, dass `15` kein Quellenordner ist.
...
Konzeptionell steht hier eine Suchtechnik: gleiche Abschnittsfrage über viele Dateien legen. `Was ich beim Bauen brauche` ist also ein Filter, keine Stimme.
...
Mich beschäftigt bei `Was ich beim Bauen brauche` genau Daniels Kritik: Die Form darf das Denken nicht fressen.
...
Tiefer betrachtet zeigt `Was ich beim Bauen brauche`, warum die heiligen Abschnitte zweischneidig sind. Sie erzwingen Reflexion, aber sie können auch Reflexion simulieren.
...
`was_ich_beim_bauen_brauche.md` existiert, weil Daniel einzelne benannte Extrahiererdateien wollte. Das ist sinnvoll, wenn man quer über die Analyse nach `Was ich beim Bauen brauche` suchen will.

Die Datei existiert nicht, damit dieser Abschnittstyp als neues Analysekapitel ohne Ursprung gelesen wird.

## Was ich beim Bauen brauche

Beim Bauen brauche ich für `Was ich beim Bauen brauche` eine harte UI-Regel: Jeder Eintrag zeigt zuerst den Quellenpfad und einen Button `Quelle öffnen`.
...
**Vision-Schicht:** `Was ich beim Bauen brauche` ist ein Navigatorregal. Es zeigt ausgeschnittene Abschnitte aus vielen Dateien nebeneinander, aber die Bedeutung bleibt in der Ursprungsdatei.

**Code-Skizze:**
```ts
interface HolySectionEntry {
  section: 'Was ich beim Bauen brauche';
  sectionKey: 'was_ich_beim_bauen_brauche';
  sourcePath: string;
  sourceTitle: string;
...
Merken will ich mir für `Was ich beim Bauen brauche`: Aggregation ist Suchhilfe, nicht Erinnerung.

Wenn ein Satz hier wichtig wirkt, ist der nächste richtige Schritt nicht Deutung, sondern Quelle öffnen.

## Dokumente gehören zusammen

`was_ich_beim_bauen_brauche.md` gehört zu seinem Abschnittstyp, zum Manifest, zum Index und zu allen Ursprungsdateien.
...
Die Resonanz von `Was ich beim Bauen brauche` ist Suchresonanz. Viele gleichartige Abschnitte nebeneinander können Muster erzeugen.
...
`Was ich beim Bauen brauche` darf diese Schichten nicht überspringen.
...
Vergessen werden soll bei `Was ich beim Bauen brauche` die Idee, dass Aggregation automatisch Tiefe erzeugt.
...
Bis dahin bleibt `was_ich_beim_bauen_brauche.md` eine manuell markierte Suchdatei mit Quellenpflicht.
