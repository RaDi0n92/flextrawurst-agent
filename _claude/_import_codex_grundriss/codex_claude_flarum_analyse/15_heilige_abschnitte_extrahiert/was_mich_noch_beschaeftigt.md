...
Ich habe `was_mich_noch_beschaeftigt.md` als Aggregation des Abschnittstyps `Was mich noch beschäftigt` gelesen. Das heißt: Die Datei liest nicht Flarum direkt, sondern sammelt ausgeschnittene Pflichtabschnitte aus vielen Analyse-Dateien.
...
Ich verstehe `Was mich noch beschäftigt` hier als Navigator. Die Datei sammelt offene Nachbeben und ungeklärte Prüfstellen.
...
Ich verstehe aus `was_mich_noch_beschaeftigt.md` nicht, ob ein einzelner Eintrag im Ursprung stark, schwach, ironisch, warnend oder nur formal war.

Die Aggregation kann Kontextnähe anzeigen, aber keine Kontextbedeutung entscheiden.

## Was mich interessiert

Mich interessiert bei `Was mich noch beschäftigt`, welche Muster über viele Dateien hinweg auftauchen. Wiederholt sich eine echte Sorge, eine Baufrage, eine Sperre, ein Missverständnis?

Aber jedes interessante Muster muss zurück in die Ursprungsdateien springen, sonst wird aus Navigation wieder Nebel.

## Was zusammenhängt und wie

`was_mich_noch_beschaeftigt.md` hängt mit allen Ursprungsdateien zusammen, aus denen seine Einträge extrahiert wurden. Es hängt außerdem mit `MANIFEST.md` und `INDEX.md` zusammen, weil diese beiden erklären müssen, dass `15` kein Quellenordner ist.
...
Konzeptionell steht hier eine Suchtechnik: gleiche Abschnittsfrage über viele Dateien legen. `Was mich noch beschäftigt` ist also ein Filter, keine Stimme.
...
Mich beschäftigt bei `Was mich noch beschäftigt` genau Daniels Kritik: Die Form darf das Denken nicht fressen.
...
Tiefer betrachtet zeigt `Was mich noch beschäftigt`, warum die heiligen Abschnitte zweischneidig sind. Sie erzwingen Reflexion, aber sie können auch Reflexion simulieren.
...
`was_mich_noch_beschaeftigt.md` existiert, weil Daniel einzelne benannte Extrahiererdateien wollte. Das ist sinnvoll, wenn man quer über die Analyse nach `Was mich noch beschäftigt` suchen will.

Die Datei existiert nicht, damit dieser Abschnittstyp als neues Analysekapitel ohne Ursprung gelesen wird.

## Was ich beim Bauen brauche

Beim Bauen brauche ich für `Was mich noch beschäftigt` eine harte UI-Regel: Jeder Eintrag zeigt zuerst den Quellenpfad und einen Button `Quelle öffnen`.
...
**Vision-Schicht:** `Was mich noch beschäftigt` ist ein Navigatorregal. Es zeigt ausgeschnittene Abschnitte aus vielen Dateien nebeneinander, aber die Bedeutung bleibt in der Ursprungsdatei.

**Code-Skizze:**
```ts
interface HolySectionEntry {
  section: 'Was mich noch beschäftigt';
  sectionKey: 'was_mich_noch_beschaeftigt';
  sourcePath: string;
  sourceTitle: string;
...
Merken will ich mir für `Was mich noch beschäftigt`: Aggregation ist Suchhilfe, nicht Erinnerung.

Wenn ein Satz hier wichtig wirkt, ist der nächste richtige Schritt nicht Deutung, sondern Quelle öffnen.

## Dokumente gehören zusammen

`was_mich_noch_beschaeftigt.md` gehört zu seinem Abschnittstyp, zum Manifest, zum Index und zu allen Ursprungsdateien.
...
Die Resonanz von `Was mich noch beschäftigt` ist Suchresonanz. Viele gleichartige Abschnitte nebeneinander können Muster erzeugen.
...
`Was mich noch beschäftigt` darf diese Schichten nicht überspringen.
...
Vergessen werden soll bei `Was mich noch beschäftigt` die Idee, dass Aggregation automatisch Tiefe erzeugt.
...
Bis dahin bleibt `was_mich_noch_beschaeftigt.md` eine manuell markierte Suchdatei mit Quellenpflicht.
