---
datum: 2026-05-12
betrifft: [wesen-einzug, identitaet, gedaechtnis, philosophie, atomizitaet, korrektur]
importable: false
---

# Spiegel: Wesen-Einzug — das Gespräch über Identität und Schwellen
...
async def wesen_neu_erschaffen(name: str, erschaffen_von: str, db):
    """Wenn Daniel sich für neue Wesen entscheidet — sauberer Start."""
    wesen = await create_wesen_neu(name, db)
    await events.append("wesen.erschaffen", {
        "wesen_id": wesen.id,
        "art": "neu",
        "erschaffen_von": erschaffen_von,
        "flarum_id": None  # kein Flarum-Ursprung
    })
    return wesen
```
