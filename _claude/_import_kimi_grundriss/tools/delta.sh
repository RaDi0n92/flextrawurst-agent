...
echo "── Neue Spiegel ───────────────────────────"
find "$KIMI/spiegel" -name "*.md" -newer "$KIMI/notizen" -mmin "-$((STUNDEN*60))" 2>/dev/null | sort | while read f; do
    echo "  + $(basename $f)"
done
echo ""

echo "── Neue Synthesen ─────────────────────────"
find "$KIMI/ideen" -name "synthese_*.md" -mmin "-$((STUNDEN*60))" 2>/dev/null | sort | while read f; do
    echo "  + $(basename $f)"
done
echo ""

echo "── Offene Ideen ───────────────────────────"
python3 /root/werkraum/_kimi/tools/ideen_scan.py 2>/dev/null
echo ""
