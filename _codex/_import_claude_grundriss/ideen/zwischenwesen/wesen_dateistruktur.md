...
**Zur offenen Kernfrage "Roh oder KI?" (oben):** Codexium2 hat für den analogen Fall (Wesen speichert selbst etwas über sich, entspricht ungefähr `wesen_selbst`/den "lebenden Dateien" hier) eine konkrete Teilantwort gebaut: **Roh, per Marker im Antworttext** (`[MERKEN: <text> | WARUM: <warum>]`), kein separater Analyse-Prompt nötig — aber **nicht automatisch übernommen**, sondern als Vorschlag mit Begründung, den der Mensch einzeln bestätigen oder ablehnen muss. Für `mag-ich.md`/`mag-ich-nicht.md`/`handlung.md` wäre dasselbe Muster naheliegend: kein teurer 4-Prompt-Analyse-Durchlauf, sondern ein einzelner Marker-Typ pro Datei-Kategorie, den das Wesen selbst im laufenden Antworttext setzen kann — und der Mensch entscheidet, bevor irgendwas verdrängt wird. Löst nebenbei auch die Verdrängungsfrage: der Mensch sieht beim Bestätigen, was durch das Limit rausfallen würde, und entscheidet mit.

**Zur Grenzen-Unterscheidung (weich/hart) oben:** bleibt unverändert gültig, auch nach der heutigen Grenzen-Sichtbarkeits-Arbeit an Codexium2/Solarius2. Diese Arbeit hat nur die **Sichtbarkeit** verbessert (ein Toggle-Klick erzeugt jetzt ein sichtbares, dauerhaftes Ereignis im Verlauf inkl. des aktuellen Grenzen-Texts, plus ein jederzeit lesbares Dropdown mit dem Referenztext) — an der zugrunde liegenden Tatsache, dass `_wesen_grenzen.md` weiterhin **reiner, unerzwungener Text** im System-Prompt ist, hat sich nichts geändert. Absolute Grenzen (Kinderinhalte usw.) werden bei Codexium2 nur bei der *Erstellung* eines Charakters serverseitig geprüft (vergleichbar mit Schicht 1+2 in `content_filter.md` hier) — nicht laufend während des Chats. Für Flüchtlinge, wo `content_filter.md` genau diese Erstellungs-Prüfung schon vorsieht, bleibt die hier gestellte Frage weiterhin offen und ungelöst: laufende Chat-Inhalte haben auch dort keinen Server-Enforcement, nur Text-Bitten.
