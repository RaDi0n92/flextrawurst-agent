...
**Zweiter Teil desselben Vorfalls — der eigentlich schwerere Fund:** der nginx-Fix allein hätte das Problem nicht vollständig gelöst. Als der Client (durch den alten 120s-Timeout) aufgab, lief die Bildbeschreibung serverseitig unbemerkt weiter — `/upload` und `/url-lesen` hatten anders als `/chat` nie eine Abbruch-Behandlung bei Verbindungsende. Das Vision-Modell blieb dadurch minutenlang aktiv (in `ps` sichtbar: über 40 Minuten akkumulierte CPU-Zeit für einen einzigen Prozess) und blockierte den einzigen Ollama-Modell-Slot (`OLLAMA_MAX_LOADED_MODELS=1`) — das Hauptmodell konnte währenddessen gar nicht mehr laden, unabhängig vom nginx-Timeout.

Fix: `AbortController` pro Anfrage an `res.on("close")` gebunden, Signal bis in `beschreibeBildMitVisionModell` (Ollama-`http.request`), `transkribiereMitWhisper` (`execFile`, killt den Whisper-Prozess wirklich) und `leseUrlMitPlaywright` (kein natives AbortSignal, daher Browser vorzeitig schliessen) durchgereicht. Getestet: Upload mit 2s Client-Timeout während laufender Bildverarbeitung — direkt danach zeigte `ollama ps` keine geladenen Modelle mehr, kein verwaister Prozess. Ollama musste zwischendurch einmal komplett neu gestartet werden, um den zum Testzeitpunkt bereits bestehenden verwaisten Prozess loszuwerden — der Fix verhindert, dass das erneut passiert.
