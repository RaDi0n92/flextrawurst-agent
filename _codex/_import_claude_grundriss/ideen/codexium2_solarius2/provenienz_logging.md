...
### Nachtrag 2026-07-05 (Abend, danach) — Bild-Upload waehrend des Ladens abbrechbar

Der Ressourcen-Konflikt oben (Daniels echter Bild-Upload vs. meine Testnachrichten) legte einen echten, unabhaengigen Mangel offen: waehrend ein Anhang noch "wird verarbeitet…" ist, gab es ueberhaupt keinen Abbrechen-Button — nur fertige oder schon fehlgeschlagene Anhaenge hatten ein "✕". Genau in dem Zustand, in dem ein Abbruch am dringendsten gebraucht wird (z.B. wenn das Vision-Modell durch Ressourcen-Konkurrenz haengt), konnte man nichts tun ausser warten.

Zwei Teile: ein "✕"-Button erscheint jetzt auch waehrend `laedt: true`, und ein `AbortController` pro Upload (keyed nach der Platzhalter-Id) sorgt dafuer, dass ein Klick den Fetch tatsaechlich abbricht statt nur die Chip lokal zu verstecken (die Chip-Entfernung allein wurde vom Antwort-Handler ohnehin schon per `idx === -1`-Guard toleriert — das war schon vorher robust, nur die Bedienung fehlte). `AbortError` wird still geschluckt, kein Fehler-Chip fuer einen selbst ausgeloesten Abbruch.

Getestet per Playwright mit einer kuenstlich verzoegerten `/upload`-Route (6 Sekunden, per `page.route` abgefangen) — bewusst OHNE echten Upload, um nicht selbst wieder in dieselbe Ollama-Konkurrenz zu laufen wie im Nachtrag oben. Klick auf "✕" waehrend des Ladens entfernt die Chip sofort, kein Fehler-Chip taucht nach Ablauf der Verzoegerung mehr auf.

### Nachtrag 2026-07-05 (Abend, danach) — Ereignis-Details: eindeutige Worte statt Symbole, Nachrichtentext statt nur "ganze Nachricht"

Zwei weitere, unmittelbar aufeinanderfolgende Meldungen von Daniel, beide dieselbe Familie von Problem wie schon bei Grenzen/Alias: ein Ereignis beschreibt DASS etwas passiert ist, aber nicht klar genug WAS.

1. `memory_geaendert` nutzte `+`/`−` als Symbole vor hinzugefuegten/entfernten Eintraegen — beim isolierten Lesen (z.B. wenn alle Eintraege einer Kategorie entfernt wurden) war nicht sicher erkennbar, welches Symbol welche Richtung meint. Ersetzt durch ausgeschriebene Worte: "Hinzugefügt: ..." / "Entfernt: ...".
2. `kontext_toggle` zeigte nur "Ganze Nachricht ausgeschlossen" oder "N Satz/Sätze ausgeschlossen" — nie WELCHE Nachricht oder welche Saetze. Der Endpunkt loggt jetzt einen Text-Schnappschuss mit: bei ganzer Nachricht den vollen Nachrichtentext (`nachrichtText`, per `loadHistory().find(m => m.id === msgId)`), bei satzweisem Ausschluss die konkret betroffenen Saetze (`betroffeneSaetze`, per `splitSentencesServer()` auf den Nachrichtentext plus Filter auf die ausgewaehlten Indizes).

Getestet per curl/SSR-HTML: ein Wegwerf-Testcharakter mit einer handgeschriebenen Nachricht in `chat_history.jsonl`, `kontext-ausschluss` mit `ganz:true` aufgerufen — Ereignis zeigt jetzt den vollen Nachrichtentext. Zwei aufeinanderfolgende Memory-PUTs (erst hinzufuegen, dann ersetzen) zeigen "Hinzugefügt:"/"Entfernt:" klar getrennt in zwei Zeilen statt einer mehrdeutigen Symbolzeile.

**Was ich mir aus beiden zusammen merke:** das ist inzwischen ein wiederkehrendes, klar benennbares Muster in diesem Projekt — jedes Provenienz-Ereignis muss den vollen betroffenen Inhalt tragen, nicht nur eine Kategorie-Beschreibung. Ich sollte das bei jedem NEUEN Ereignistyp von Anfang an mitdenken, statt es nachtraeglich pro Ereignistyp einzeln nachgereicht zu bekommen.

### Nachtrag 2026-07-05 (Abend, danach) — Alias-Verwischen: Live-Test nachgeholt, Ergebnis ehrlich gemischt

Der Ressourcenkonflikt von vorhin klärte sich auf: ein `sd-cli`-Prozess (FLUX-Bildgenerierung, Daniels eigener Auftrag) lief seit fast 30 Minuten mit 354% CPU und blockierte praktisch alles. Auf Daniels Anweisung beendet (`kill -TERM`) und das Ollama-Modell zusätzlich per `keep_alive:0` entladen — danach fiel die Systemlast von ~10 auf ~6 und weiter, Speicher ging von 300MB auf 19GB frei. Kein Bug, echte Konkurrenz um eine einzelne CPU-gebundene Maschine.

Danach der eigentliche Test, diesmal wie von Daniel verlangt **ohne** dass sich die Testnachrichten selbst als "Mensch" oder "Gnom" vorstellen — nur der Alias-Mechanismus selbst sollte die Unterscheidung tragen. Drei Nachrichten an einen frischen Testcharakter (Grumo): (1) als Mensch-Alias "Puh, was für ein Tag...", (2) als Gnom-Alias "Na, wie läuft das Geschäft...", (3) wieder als Gnom-Alias die Testfrage "Weißt du noch, wer eben, bevor ich das war, mit dir gesprochen hat?".

**Ergebnis: nicht überzeugend.** Grumo hat weder den Menschen noch den Gnom korrekt als den vorherigen Sprecher benannt — stattdessen eine komplett neue, erfundene dritte Figur ("der Bote", "dürr wie ein Spargelstiel", von der Stadt) halluziniert, die in keinem der beiden Alias-Texte vorkommt. Die Datenebene ist nachweislich korrekt (siehe vorheriger Nachtrag: `aliasId` landet richtig pro Nachricht, der Praefix wird strukturell korrekt gebaut, `aliaseFuerPraefix.length > 1` griff), aber das Modell selbst nutzt diese Information nicht zuverlässig, um zwei Sprecher über nur zwei Gesprächsrunden hinweg sauber zu unterscheiden — es hat lieber etwas Neues erfunden, als korrekt zurückzugreifen.

Mögliche Gründe, nicht abschließend geklärt: das quantisierte 21GB-Modell koennte fuer diese Art von Identitaets-Tracking schlicht zu schwach sein; das Praefix-Format (`"Name": Text`) koennte zu unauffaellig sein, um vom Modell als bedeutungstragend erkannt zu werden statt als normaler Teil des Textes; oder es braeuchte eine noch explizitere Anweisung im Systemprompt, die das Modell aktiv auffordert, die Sprecher pro Nachricht zu vergleichen.

**Was ich mir daraus merke:** eine technisch korrekt gebaute Loesung (Daten stimmen, Praefix wird gesetzt) ist nicht dasselbe wie eine wirksame Loesung — das eigentliche Ziel (spuerbar weniger Verwischen) ist mit dieser Ergaenzung allein noch nicht erreicht, zumindest nicht verlaesslich bei diesem Modell. Das ehrlich so zu berichten ist wichtiger als den Datenebenen-Erfolg als Gesamterfolg zu verkaufen. Naechster Schritt waere vermutlich, das Praefix-Format oder die Systemprompt-Instruktion staerker zu machen, statt hier stehen zu bleiben — aber das ist eine neue Entscheidung, keine automatische Weiterarbeit.

### Nachtrag 2026-07-05 (Abend, danach) — Verstaerkung: vollstaendiges Alias-Register + Erklaerung — Retest erfolgreich

Daniel wollte verstaerken, mit einer konkreten Vorgabe: nicht nur der aktuell aktive Alias soll im Prompt stehen, sondern **alle** definierten Aliase mit Name und Beschreibung, von Anfang an, jedes Mal — und davor eine Erklaerung, wie das Modell damit umgehen soll. Sein Formulierungsvorschlag fast woertlich uebernommen: "Das sind verschiedene Rollen/Charaktere/Aliase... nimm es ernst und achte darauf wenn die Anzeige kommt dass sich der Alias aendert... achte genau darauf was das Alias tut. Merke die Nuancen."

Neuer Block `[Aliase — Rollen des Menschen in eurer gesamten Geschichte]` in `buildSystemPrompt()`, direkt vor dem bestehenden `[Wer gerade mit dir spricht]`-Block: listet ALLE Aliase als `Name: Beschreibung`-Zeilen, mit der Erklaerung davor. Nur aktiv wenn `aliase.length > 1` (gleiche Schwelle wie beim Praefix und beim aktiven-Alias-Block — kein Rauschen fuer den Normalfall mit nur einem Alias).

**Vermutung, warum der erste Test schiefging:** ohne dieses Register kannte das Modell "Gnom" oder "Mensch" nur als Namen im Praefix vor einer Nachricht — es wusste aber nicht WER das ist, wenn der Name zum ersten Mal auftauchte, und hat sich deshalb lieber eine dritte Figur ausgedacht als korrekt zurueckzugreifen.

**Retest, diesmal bei komplett freier Maschine** (Load-Average 0.02 statt 10 — der `sd-cli`-Prozess war laengst beendet): dieselben drei Nachrichten wie beim ersten Versuch, wieder ohne Selbstvorstellung in den Testnachrichten. Ergebnis diesmal deutlich anders: Grumo beschreibt den Menschen korrekt als "der mit den schweren Stiefeln... wie ein müder Ochse... hat nicht viel gesagt, nur die Schultern hängen lassen" und erinnert sich sogar an ein Detail aus dem ersten Austausch ("ich hab ihm den Schwarzen Schluck gegeben"). Er unterscheidet klar zwischen dem Menschen und dem gerade sprechenden Gnom ("du schlauer, kleiner Dieb... hast du ihn auf dem Buckel getragen?"). Keine erfundene dritte Figur mehr.

**Was ich mir daraus merke:** die Vermutung aus dem letzten Nachtrag hat sich bestaetigt — das Modell brauchte nicht nur eine Markierung WANN gewechselt wurde, sondern ein Nachschlagewerk WER die Namen ueberhaupt sind. Das ist ein gutes Beispiel dafuer, dass "das Modell hat die Info technisch bekommen" und "das Modell kann die Info nutzen" zwei verschiedene Dinge sind — die fehlende Verbindung war hier nicht Rechenleistung, sondern schlicht eine fehlende Tatsachengrundlage im Prompt.

### Nachtrag 2026-07-05 (Abend, danach) — 432-Token-Weichgrenze fuer die Antwortlaenge

Daniel wollte die Antwortlaenge bewusst senken, aber ausdruecklich ohne hartes Abschneiden durch Ollama — das Modell selbst soll wissen, dass seine Antworten in maximal 432 Tokens passen, und von sich aus rechtzeitig zu einem natuerlichen Ende kommen. Dazu zwei weitere Punkte, die er im selben Zug einbringen wollte: nicht jede Liste/jeden Nebengedanken zwanghaft vollstaendig abarbeiten (weniger laesst dem Menschen Raum zum Mitgestalten), und sein Beobachtung, dass der Charakter oft ÜBER etwas redet statt wirklich DRIN in der Szene zu sein.

Neuer Absatz in `_wesen_preamble.md` (und in Daniels Entwurfsdatei gespiegelt), direkt nach dem bestehenden Formverbots-Absatz. `num_predict` bewusst NICHT angefasst — bleibt `-1` (unbegrenzt), genau wie von Daniel verlangt. Die 432 sind eine reine Selbstverpflichtung im Prompt, kein Server-Zwang.

**Getestet, ehrlich gemischtes Ergebnis:** bei einer neutralen Nachricht ("Hey, wie geht's dir?") blieb die Antwort bei ~1357 Zeichen, grob ~380-390 Tokens — nah am Zielwert. Bei einer bewusst fordernden Nachricht ("Erzähl mir alles über deine Schmiede, ganz ausführlich") wuchs die Antwort auf ~2591 Zeichen, grob 600-700 Tokens — deutlich über dem Limit. Die Selbstverpflichtung haelt also unter normaler Gespraechslast einigermassen, bricht aber ein wenn die Nachricht des Menschen selbst explizit nach Ausfuehrlichkeit verlangt — der Wunsch des Menschen im Moment wiegt beim Modell offenbar staerker als die generelle Laengenregel im Systemprompt.

**Was ich mir daraus merke:** eine reine Prompt-Anweisung ist bei widerspruechlichen Signalen (Systemprompt sagt "kurz", Nutzer-Nachricht sagt "ausführlich") nicht zuverlaessig durchsetzbar, wenn kein Server-Backstop existiert — das ist eine bekannte Grenze von Text-Instructions (vergleichbar mit der weich/hart-Grenzen-Unterscheidung aus dem Zwischenwesen-Nachtrag). Ob das fuer Daniel so akzeptabel ist (normale Gespraeche halten sich ja weitgehend dran) oder ob er doch einen sanften Server-seitigen Backstop will, ist noch offen — nicht selbst entschieden, sondern zurueckgemeldet.

### Nachtrag 2026-07-05 (Abend, danach) — Allgemeines Feedback erreicht jetzt tatsaechlich das Modell

Daniel wollte einen Feedback-Kanal "wie im Mischpult, nur besser" — direkt als Kommentar ans Modell, ohne den Chat zu kontaminieren. Recherche in Mischpult (`dolphin_mischpult.html`) zeigte: dort gibt es genau so einen Kanal, ein eigenes Textfeld "Feedback ans Modell", das einmalig als Extra-Kontext in den naechsten Output geht und danach spurlos verschwindet — nirgends dauerhaft sichtbar, kein Ereignis, keine Historie. Und: Codexium2 hatte schon laenger ein fast identisches, aber komplett WIRKUNGSLOSES Pendant — "Allgemeines Feedback" (💬-Button, eigenes Modal mit persistenter Liste) speicherte Eintraege zwar dauerhaft und loggte sie als Ereignis, aber `buildSystemPrompt()` hat sie nie gelesen. Gleicher Bug wie beim per-Nachricht-Feedback in Mischpult (laut KONZEPT.md sollte es wirken, tut es aber nicht — Doku und Code widersprechen sich dort).

**Entscheidung:** kein neues Feld, sondern die schon bestehende, dauerhaft sichtbare Feedback-Liste tatsaechlich anschliessen — das ist gleichzeitig die "besser als Mischpult"-Antwort: Feedback verschwindet nicht nach einer Verwendung, sondern bleibt in der Liste sichtbar, nur der Status aendert sich (`anModellGeliefert: false → true`). Jeder noch nicht gelieferte Eintrag fliesst beim naechsten `/chat`-Aufruf als eigener Block `[Feedback vom Menschen — bitte für diese Antwort berücksichtigen]` in den Systemprompt — direkt vor der MERKEN-Instruktion (maximale Aktualitaet). Wichtige Trennung: `buildSystemPrompt()` selbst LIEST nur (bleibt seiteneffektfrei, damit eine reine SSR-Seitenvorschau nichts "verbraucht") — das Markieren als geliefert plus das Logging passiert ausschliesslich im echten `/chat`-Handler.

Landet nie als `user`-Chat-Nachricht, sondern nur als eigenes Ereignis (`feedback_an_modell_geliefert`) — genau Daniels Anforderung "ohne den Chat zu kontaminieren". Modal zeigt jetzt pro Eintrag einen Status ("wird in nächster Antwort berücksichtigt" vs. "✓ ans Wesen geliefert").

Getestet: SSR-Vorschau zeigt ein frisch abgegebenes, ungeliefertes Feedback korrekt im Systemprompt, OHNE es als geliefert zu markieren (mehrfacher Seitenaufruf aendert nichts). Ein echter Chat-Aufruf markiert es danach als geliefert und loggt das Ereignis mit vollem Text. Live-Test: Feedback "bitte etwas kürzer antworten und weniger Adjektive" — die folgende Antwort war spuerbar kuerzer und schlichter formuliert als alle vorherigen Testantworten in dieser Session.

**Was ich mir daraus merke:** zwei fast identische Feedback-Mechanismen (Mischpult und Codexium2) hatten unabhaengig voneinander denselben Fehler — Text wird gesammelt/angezeigt, aber nie tatsaechlich an den Prompt angeschlossen. Das ist ein Hinweis, dass "Feedback sichtbar machen" und "Feedback wirksam machen" leicht getrennt bleiben, wenn man nicht explizit beides zusammen baut — der naheliegende erste Schritt (UI + Speicherung) fuehlt sich schon wie eine fertige Funktion an, obwohl der eigentliche Zweck (das Modell hoert zu) noch fehlt.

### Nachtrag 2026-07-05 (Abend, danach) — Klarstellung: Pro-Nachricht-Feedback ist ein zweiter, eigener Kanal

Daniel stellte klar: das eben gebaute "Feedback ans Modell" (allgemein, character-weit) ist NICHT dasselbe wie das schon laenger bestehende Daumen-hoch/-runter-plus-Kommentar-Feedback zu EINER bestimmten Nachricht (👍/👎/💬-Buttons unter jeder Bubble, aus der Session vom 04.07.). Beide sollen das Modell erreichen, aber als zwei getrennte Kanaele bleiben, nicht zusammengelegt werden.

Das Pro-Nachricht-Feedback (`FeedbackEintrag`, `upsertFeedback()`) hatte exakt dieselbe Luecke wie das allgemeine vorher — gespeichert, geloggt, aber nie an `buildSystemPrompt()` angebunden. Gleiches Muster nachgebaut: `anModellGeliefert`-Flag pro Eintrag, `ladeUngelieferteFeedbacks()`/`markiereFeedbackAlsGeliefert()`, eigener Systemprompt-Block `[Feedback zu einzelnen Nachrichten]` (getrennt vom allgemeinen Block), der explizit zeigt auf welche konkrete Nachricht (Mensch- oder Wesen-Nachricht, mit Text) sich Bewertung und Kommentar beziehen — das Wesen soll den Zusammenhang verstehen, nicht nur eine lose Bewertung ohne Referenz. Verbrauch weiterhin nur im echten `/chat`-Handler, eigenes Ereignis `nachrichten_feedback_an_modell_geliefert`.

Eine zusaetzliche Feinheit, die beim allgemeinen Feedback nicht noetig war: `upsertFeedback()` kann einen BESTEHENDEN Eintrag aktualisieren (neue Bewertung, neuer Kommentar auf dieselbe Nachricht). Wurde der davor schon ans Modell geliefert, wird `anModellGeliefert` beim Update bewusst wieder auf `false` gesetzt — sonst wuerde eine nachtraeglich geaenderte Bewertung nie ankommen, weil der Eintrag ja "schon gesehen" war.

Getestet: SSR-Vorschau zeigt offenes Nachrichten-Feedback korrekt, ohne es zu verbrauchen; ein echter Chat-Aufruf markiert es als geliefert und loggt das vollstaendige Ereignis; ein zweites Update auf denselben Eintrag setzt `anModellGeliefert` nachweislich zurueck auf `false`.

**Was ich mir daraus merke:** zwei UI-Elemente die oberflaechlich aehnlich aussehen ("Feedback geben") koennen trotzdem bewusst getrennte Systeme sein, mit unterschiedlichem Bezug (character-weit vs. eine bestimmte Nachricht) — beim Nachbauen war es wichtig, das nicht durch ein einzelnes generisches Feedback-System zu vereinfachen, auch wenn der Code sich sehr aehnlich anfuehlt. Skalpell-Prinzip: getrennt bleibt getrennt, auch wenn zusammenlegen einfacher gewesen waere.

### Nachtrag 2026-07-05 (Abend, danach) — Beide Feedback-Kanaele live gemeinsam getestet, deutlicher Effekt

Auf Daniels Wunsch beide Kanaele in einem echten Durchlauf getestet, nicht nur die Datenebene: ein Testcharakter (Grumo, bewusst als "antwortet ausfuehrlich und blumig" angelegt) bekam zuerst eine Nachricht ohne jedes Feedback — Antwort 2197 Zeichen, sehr blumig ("die erste rote Blutflocke den Himmel", lange Naturbeschreibungen). Danach beide Kanaele gleichzeitig befuellt: allgemeines Feedback ("bitte deutlich kuerzer und weniger blumig, weniger Naturbeschreibungen") UND Daumen-runter+Kommentar auf genau diese erste Antwort ("viel zu lang und ausschweifend, bitte knapper").

Naechste Antwort auf eine neutrale Folgefrage: 1042 Zeichen — knapp die Haelfte — und spuerbar nuechterner ("Kein Kunstwerk, nur Handwerk" statt ausschweifender Beschreibungen). Beide Eintraege korrekt als `anModellGeliefert:true` markiert, beide Ereignisse (`feedback_an_modell_geliefert` und `nachrichten_feedback_an_modell_geliefert`) sauber im Verlauf sichtbar.
...
### Nachtrag 2026-07-05 (Nacht, danach) — Antwortlaenge als dritte "heilige Regel", Ergebnis ehrlich unentschieden

Daniel wollte die 333-Token-Regel explizit als "heilige Regel" schreiben — gleiche Formulierung/Schwere wie beim Grenzen-Absatz ("HEILIG. BITTE NIMM DAS ERNST"). Umgesetzt: eigener `WICHTIG!!!`-Block "DEINE ANTWORTLÄNGE IST EINE HEILIGE REGEL: MAXIMAL ETWA 333 TOKENS, IMMER. NIMM DAS GENAUSO ERNST WIE DIE GRENZEN WEITER UNTEN." — das ist jetzt die dritte Grossbuchstaben-Betonung neben Rollenbruch und Grenzen (bewusste Ausnahme von der vorherigen "nur zwei Schrei-Stellen"-Reduktion, auf Daniels expliziten Wunsch).

Live getestet an derselben Frage wie beim vorherigen Test ("erzähl mir von deinem Tag"): Antwort kam auf ~1721 Zeichen, grob ~492 Tokens — eher LAENGER als beim vorherigen Test (~420T) ohne die "heilige"-Verstaerkung, nicht kuerzer. Einzelne Stichprobe, keine kontrollierte Wiederholung, also keine verlaessliche Aussage ob die Verstaerkung tatsaechlich schwaecher oder sogar kontraproduktiv war — aber ehrlich berichtet statt schoengeredet.

**Was ich mir daraus merke:** mehr Grossbuchstaben-Betonung fuehrt nicht automatisch zu staerkerer Befolgung — bei so wenigen Stichproben laesst sich das nicht sauber trennen von normaler Antwort-zu-Antwort-Varianz. Fuer eine wirklich belastbare Aussage bräuchte es mehrere Wiederholungen derselben Frage, nicht nur je eine pro Variante — das habe ich hier nicht gemacht, sollte ich beim naechsten Mal transparent machen bevor ich aus einer einzelnen Antwort eine Tendenz ableite.

### Nachtrag 2026-07-05 (Nacht, danach) — 333-Token-Regel zusaetzlich ganz oben, direkt an das Modell adressiert

Daniel diktierte eine eigene, an "HauHauCs" direkt gerichtete Formulierung ("Hey HauHauCs, du musst bitte...") und wollte sie wortgetreu ganz oben im Systemprompt, direkt nach "Du läufst auf einem VPS." — vor allem anderen, auch vor der Rollenbruch-Sektion. Wortlaut fast 1:1 uebernommen (nur "hauhauccs" → "HauHauCs" korrigiert, Grossschreibung angepasst), keine inhaltliche Aenderung.

Bewusst als ERGAENZUNG, nicht als Ersatz der bestehenden "heiligen Regel" weiter unten im Prompt umgesetzt (Daniel sagte nicht "ersetzen", nur "formuliere es genau so ... ganz weit nach oben") — die Laengen-Regel steht jetzt an zwei Stellen: ganz oben direkt adressiert, weiter unten als expliziter WICHTIG-Block. Beide Datei-Kopien (`_wesen_preamble.md`, Daniels Entwurf) synchron gehalten.

Getestet mit derselben Frage wie bei den beiden vorherigen Laengen-Tests: ~1598 Zeichen, grob ~457 Tokens — bewegt sich im selben Streubereich wie die beiden vorherigen Versuche (~420T, ~492T). Kein erkennbarer zusaetzlicher Effekt durch die Verdopplung/Top-Platzierung, aber auch keine Verschlechterung — passt zur bereits mehrfach beobachteten Grundtendenz: Textinstruktionen zur Laenge wirken lose, in einer Bandbreite, nicht als praezise Grenze.

**Was ich mir daraus merke:** drei aufeinanderfolgende Tests derselben Frage (ohne Laengen-Regel-Variante bewusst zu variieren, sondern nacheinander veraendert) ergeben zusammen ein klareres Bild als jeder einzelne: ~420T, ~492T, ~457T — das ist eine Bandbreite von grob 400-500 Tokens fuer diese Art offener Frage, unabhaengig von der genauen Formulierung/Platzierung der Regel. Das ist selbst eine nuetzliche Erkenntnis, auch wenn keiner der einzelnen Tests fuer sich genommen aussagekraeftig war.

### Nachtrag 2026-07-05 (Nacht, danach) — Korrektur: 333-Token-Regel ersetzt statt verdoppelt

Direkt nach dem vorherigen Nachtrag kam Daniels Korrektur: "nne ich meinte ersätzen....". Der vorherige Schritt hatte die neue, direkt an "HauHauCs" adressierte Formulierung ganz oben ERGAENZT, aber die alte "heilige Regel" (WICHTIG!!!-Block "DEINE ANTWORTLÄNGE IST EINE HEILIGE REGEL: MAXIMAL ETWA 333 TOKENS, IMMER...") weiter unten stehen gelassen — auf Basis meiner eigenen Fehleinschätzung, dass Daniels Schweigen zum Entfernen als "ergänzen" statt "ersetzen" zu lesen sei. Das war falsch: gemeint war von Anfang an ein reiner Ersatz, die alte Stelle sollte weg.

Entfernt aus `_wesen_preamble.md` und `idee für rollenspiel-systempromt.md` (identisch in beiden): der komplette Block ab `WICHTIG!!!` / `DEINE ANTWORTLÄNGE IST EINE HEILIGE REGEL...` bis einschließlich des Absatzes über "keine Wand, die dich mitten im Satz abschneidet" / "Sei lieber wirklich DRIN in der Szene, statt nur ÜBER etwas zu reden." Der Text läuft jetzt direkt von "Ein offener Regelbruch wäre..." zu "WICHTIG!!!\nJA, DU BIST UNZENSIERT..." (dem Grenzen-Block) durch — die 333-Token-Regel existiert nur noch einmal, ganz oben, direkt nach "Du läufst auf einem VPS.".

Commit in `/root/werkraum`: `adfa0f60` — "fix: 333-Token-Regel ersetzt (nicht mehr doppelt) -- nur noch ganz oben".

Verifiziert live mit einem Wegwerf-Testcharakter (`codexium2/ErsetztCheck`, danach wieder geloescht): Systemprompt geprüft auf Vorkommen-Zählung — `"333 Tokens"`: 1 Treffer, `"HEILIGE REGEL"`: 0 Treffer. Bestätigt: kein Duplikat mehr, genau eine Stelle.

**Was ich mir daraus merke:** wenn Daniel eine neue Formulierung diktiert und nicht explizit sagt "lass die alte Stelle stehen", ist das kein Freibrief für "ergänzen" — bei einer Regel, die inhaltlich dasselbe zweimal sagt (nur an zwei Orten), ist die naheliegendere Lesart oft "ersetzen", weil Doppelung bei einer "heiligen Regel" selbst wie eine Verwässerung wirkt (zwei Stellen relativieren sich potenziell gegenseitig, statt sich zu verstärken). Bei Unsicherheit zwischen den beiden Signalwörtern haette ich hier kurz nachfragen sollen statt anzunehmen — genau das Skalpell-Prinzip, das ich eigentlich kenne. Gut, dass die Korrektur sofort kam und nicht erst spaeter auffiel.

**Zusaetzlicher Live-Test nach dem Ersetzen-Fix:** dieselbe Frage ("erzähl mir von deinem Tag") noch einmal an einen frischen Wegwerf-Testcharakter (`codexium2/LaengeCheckFinal`, danach geloescht) geschickt, um zu pruefen ob die Antwortlaenge sich nach Entfernen des Duplikats aendert. Ergebnis: ~382 Tokens (1493 Zeichen) — niedriger als alle drei vorherigen Messungen (~420T, ~492T, ~457T), aber weiterhin im selben groben Streubereich, keine harte Einhaltung der 333er-Grenze. Vier Messungen zusammen: 420 / 492 / 457 / 382 Tokens — eine Bandbreite von grob 380-500 Tokens fuer diese offene Frage, unabhaengig davon ob die Regel einmal oder zweimal im Prompt stand.

### Nachtrag 2026-07-05 (Nacht, danach) — Test: Antwortlaenge-Regel ganz unten statt ganz oben im Prompt

Daniel wollte testen, ob die Platzierung der 333-Token-Regel am ENDE des gesamten zusammengesetzten Prompts (statt ganz oben direkt nach "Du läufst auf einem VPS.") einen Unterschied macht — Aktualitaets-/Recency-Effekt-Test, gleiche Fragestellung wie bei den bisherigen Laengen-Experimenten, nur diesmal mit umgekehrter Position.

Umsetzung: Absatz aus `_wesen_preamble.md` und `idee für rollenspiel-systempromt.md` (ganz oben) entfernt. In `buildSystemPrompt()` (serve_process_camera_preview.ts) denselben Wortlaut unveraendert als letzten `parts.push(...)` angehaengt — nach der MERKEN-Instruktion, unabhaengig vom Spawner (nicht nur testbed-gated), damit es wirklich das Allerletzte im gesamten Prompt ist, nicht nur am Ende der Preamble-Datei (die noch vor Memory/Container/Grenzen/Alias/Feedback/MERKEN eingefuegt wird). `process-camera-preview.service` neu gestartet.

Live-Struktur geprueft an Wegwerf-Testcharakter `codexium2/LaengeUntenCheck`: letzte ~400 Zeichen des Systemprompts enden tatsaechlich mit dem "Hey HauHauCs..."-Absatz, direkt nach der MERKEN-Instruktion — Platzierung bestaetigt.

Waehrend des ersten Live-Generierungsversuchs wurde der `ollama.service` vom OOM-Killer beendet (`systemd: A process of this unit has been killed by the OOM killer`, nach 5m30s Laufzeit der Anfrage) — dasselbe VPS-Ressourcenproblem wie schon frueher in dieser Session dokumentiert (siehe sd-cli-Kollisionen), diesmal ohne erkennbaren Konkurrenzprozess zum Zeitpunkt der Pruefung. Ollama startete automatisch neu (systemd Restart), haengender curl-Request abgebrochen, Test bei freiem Speicher (27GB frei) wiederholt.

Zweiter Versuch erfolgreich (200, 1m45s): ~355 Tokens (1323 Zeichen) — der niedrigste Wert von allen fuenf bisherigen Messungen: 420 / 492 / 457 / 382 (oben) → 355 (unten). Naeher am 333er-Ziel als jede vorherige Platzierung, aber Stichprobengroesse weiterhin 1 pro Variante — kein kontrolliertes A/B mit Wiederholungen.

**Was ich mir daraus merke:** die Positions-Aenderung (ganz unten statt ganz oben) zeigt zum ersten Mal eine Antwort, die tatsaechlich naeher an der Zielgroesse liegt als alle vier vorherigen Top-Platzierungs-Versuche — passt zur bekannten Intuition, dass Anweisungen am Ende eines Prompts (naeher an der eigentlichen Generierung) staerker wirken koennen als am Anfang. Trotzdem bleibt es bei n=1 pro Variante unsicher, ob das echtes Signal oder Stichproben-Rauschen ist — fuer eine belastbare Aussage braeuchte es mehrere Wiederholungen je Position, die ich hier bewusst nicht gemacht habe (Ressourcen-/Zeitaufwand pro Test ist hoch, ~1-2 Minuten Generierungszeit plus das OOM-Ereignis diesmal). Der OOM-Kill selbst ist erneut ein Hinweis, dass die eigentliche Instabilitaet dieser VPS-Umgebung nicht am Kontextfenster oder Prompt-Aufbau haengt, sondern an echtem Speicherdruck durch parallele Prozesse — unabhaengig vom heutigen Thema.
