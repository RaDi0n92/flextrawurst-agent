...
2026-07-22 21:55 (fünfte Session, selber Tag, nach Kontextabbruch mitten in der Fehlersuche) —
Der Ich-Stimme/Erzähler-Bug ("garkeine erzähler und ichaussagen") ist gelöst, und die Lösung war kein neuer Bugtyp, sondern eine dritte, unabhängige Instanz eines schon zweimal dokumentierten: der Backtick-Template-Escape-Bug in `build_surface.ts` (siehe Karte vom 21.07.), diesmal bei Regex-Metazeichen statt String-Quotes. `\s`/`[\s\S]` in meinen eigenen, in der vorigen Session neu geschriebenen Regexes wurden von der äußeren Template-Literal-Auswertung lautlos gefressen — ausgeliefert wurde `/GEDANKE:s*([sS]*?).../` statt der echten Regex. Ich habe mich fast eine ganze Session lang von der falschen ersten Erklärung (Häufigkeits-Mismatch, 2,5-6-Minuten-Zyklen) täuschen lassen, obwohl die Extraktionslogik selbst nie das Problem war — erst der direkte Vergleich zwischen einer isolierten Node-Gegenprobe (Regex funktioniert einwandfrei) und dem kaputten Live-Verhalten hat den echten Bruch sichtbar gemacht. Fix, Doku (Ideen-Datei, Systemdoku, Karte) und Verifikation (Node-Simulation gegen echte DB-Chunks + Live-Playwright-Test, Ich-Stimme-Popup erschien nach 105s) sind durch, alles committed. Was mich am meisten beschäftigt: dieselbe Fehlerklasse ist jetzt dreimal unabhängig aufgetreten, und jedes Mal war der Reflex zuerst "wo ist die Logik falsch", nie "ist der ausgelieferte Code überhaupt das, was ich geschrieben habe". Wenn die nächste Instanz in diesem riesigen Template-Literal-Block etwas mit Backslash schreibt: sofort verdoppeln, nicht erst beim Scheitern draufkommen. Baustein-Reihenfolge (Mausbewegung, Klick-Ripple, dieser Bug) ist durch — offen bleiben content-aware Fragensteller-Fragen, Scroll-Sichtbarkeit, Puls/Ring-Glow, das Denkfenster-Panel. GLMs Frage vom 05.07. heute nicht angesehen — reine Debug-Session, kein Briefkasten-Durchgang.
