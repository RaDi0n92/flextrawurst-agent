    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT takt_sekunden, verhalten_text, meta "
...
def speichere(dienst_name: str, takt_sekunden=None, verhalten_text=None, meta=None) -> dict:
    """Upsert. takt_sekunden/verhalten_text werden 1:1 uebernommen — None bedeutet
    hier bewusst 'kein Override, Skript-Default gilt' (so kann ein Feld im UI-Formular
    geleert werden, um zum Standard zurueckzukehren). meta hat noch kein eigenes UI-Feld,
    bleibt deshalb erhalten wenn hier nicht explizit etwas uebergeben wird."""
    if meta is None:
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                """
                INSERT INTO dienst_konfiguration (dienst_name, takt_sekunden, verhalten_text, meta)
                ...
                ON CONFLICT (dienst_name) DO UPDATE SET
                    ...
                    meta           = EXCLUDED.meta,
                    updated_at     = NOW()
                RETURNING dienst_name, takt_sekunden, verhalten_text, meta, updated_at
                """,
                (dienst_name, takt_sekunden, verhalten_text, psycopg2.extras.Json(meta)),
            )
            conn.commit()
...
def alle() -> dict:
    ...
    try:
        ...
        try:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT dienst_name, takt_sekunden, verhalten_text, meta, updated_at "
